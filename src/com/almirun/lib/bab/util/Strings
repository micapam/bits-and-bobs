package com.almirun.lib.bab.util {
  ...
  /**
	...
	public class Strings {
		...
		public static function isEmpty(str:String = null):Boolean {
		...
		public static function truncate(str:String, maxChars:int = 100,
				ellipsis:String = "..."):String
		{
			return str.length > maxChars
				? str.substr(0, maxChars - ellipsis.length) + ellipsis
				: str;
		}
		...
		public static function forceFit(str:String, size:int,
				padding:String = " "):String
		{
			...
			if (size < 0) 
			{
				throw new ArgumentError("Size argument must be positive"); 
			}
			else if (str.length > size) 
			{
				fitted = str.substr(0, size);
			}
			else 
			{
				fitted = str;
